use crate::domain::chat::{
    ChatDomainSeed, CircleItem, CircleStatus, CircleType, ContactItem, GroupMember, GroupProfile,
    GroupRole, MessageAuthor, MessageDeliveryStatus, MessageItem, MessageKind, MessageSyncSource,
...
impl ChatRepository for SeedChatRepository {
    ...
    fn save_domain_seed(&self, _seed: ChatDomainSeed) -> Result<(), String> {
        Err("seed repository is read-only".into())
    }
}

fn load_circles() -> Vec<CircleItem> {
    vec![
        CircleItem {
            id: "main-circle".into(),
...
fn load_contacts() -> Vec<ContactItem> {
    vec![
        contact(
            "alice-contact",
            "Alice Chen",
...
fn load_groups() -> Vec<GroupProfile> {
    vec![
        GroupProfile {
            session_id: "design".into(),
            name: "Design Circle".into(),
            description: "Core product and design discussion space for shell parity work.".into(),
            members: vec![
        ...
        GroupProfile {
            ...
            name: "Studio Circle".into(),
            description:
                "Private relay group used to validate paid-circle behavior and onboarding.".into(),
            members: vec![
...
fn load_message_store() -> HashMap<String, Vec<MessageItem>> {
...
fn text_message(id: &str, author: MessageAuthor, body: &str, time: &str) -> MessageItem {
    ...
    MessageItem {
        ...
        delivery_status: delivery_status_for_author(&author),
        author,
        body: body.into(),
        time: time.into(),
        meta: None,
...
fn delivery_status_for_author(author: &MessageAuthor) -> Option<MessageDeliveryStatus> {
    match author {
        MessageAuthor::Me => Some(MessageDeliveryStatus::Sent),
        MessageAuthor::Peer | MessageAuthor::System => None,
    }
}

fn seed_remote_message_id(id: &str) -> String {
    format!("seed-relay:{id}")
}
