...
- [~] 消息模型扩展: 已补 `deliveryStatus + remoteId + syncSource + ackedAt` 基础合同与展示，也已补最小 `reply`、`mention`、`message detail`、file attachment send、image message 和 video message 交互；当前 file/image/video attachment 已从 SQLite 内联 `data:` 预览迁到原生 `chat-media/` 文件存储，消息 `meta` 现可持久化 `label + localPath`，并开始兼容 `remoteUrl` 这类后续真实媒体合同字段，timeline/detail 也可按本地 asset URL 或远端 URL 回放；Rust 侧 duplicate merge / relay echo 也已能按字段合并 `localPath + remoteUrl`，不会再互相覆盖；同时已新增最小 native cache/download 边界，可把 remote-only media 下载进 `chat-media/` 并回写 `localPath`，也新增了显式的 `update_chat_message_media_remote_url` mutation 来把现有本地媒体消息提升成 `localPath + remoteUrl` 或 `remoteUrl-only` 的结构化 meta；现在 outbound media 也已不再依赖 runtime 内部临时 HTTP server，而是先走统一 native `media_upload` 抽象产出 `remoteUrl`，默认 fallback 到主进程 file-server，配置环境变量或 settings 时可做真实 `filedrop` multipart upload，也已支持 `NIP-96 + NIP-98` upload、复用当前 authenticated runtime 的 `Blossom` upload，以及 env-backed `MinIO` upload，且新的 `advancedPreferences.mediaUploadDriver / mediaUploadEndpoint` 与 settings Advanced 面板已经把这层 backend 选择持久化进 shell store，再通过现有 merge/receipt 路径回写进消息记录，outbound media dispatch 也已开始按 `localPath` 持久化最小判重；domain mutation 后也会做最小 orphan cleanup；但这层真实 upload 仍缺少消息级 upload status、更完整的引用计数/GC、reaction、private/presigned MinIO 适配或 dedicated file server 管理页面；更完整的远端事件模型仍缺失。
...
1. 先把当前已经落到原生 `chat-media/` 的 `file / image / video` 本地资产，继续收敛成真实 `upload / download / media URL` 合同；当前 native `media_upload` 已经把 “本机 file-server fallback”、真实 multipart upload、`NIP-96 + NIP-98 + Blossom + MinIO`，以及 `advancedPreferences.mediaUploadDriver / mediaUploadEndpoint` 这层持久化配置都从 transport/runtime 里抽出来，所以下一步应直接补消息级 upload status、private/presigned MinIO 适配，以及更完整的 media 生命周期治理；若 upload 最终迁到 transport 外，再复用 `update_chat_message_media_remote_url` mutation 产出 `remoteUrl`，然后再把现在仅有的 orphan cleanup 推进到更完整的引用计数/GC 边界。
