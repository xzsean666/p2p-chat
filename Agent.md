...
2. 当前产品范围明确收敛为: `简单 P2P 聊天 MVP`。凡是不属于 MVP 聊天闭环的能力，即使 `tmp/xchat-app-main` 里存在，也不能抢在主链之前实现。
3. 凡是需要独立后端、圈子服务端、对象存储服务、付款/订阅服务或任何额外服务端接口配合的任务，一律暂停，除非用户明确重新开启。
4. 暂停范围至少包括:
...
5. 若某项功能必须依赖后端才能成立，不要继续扩展该方向，也不要临时发明新的 mock backend、env backend 或服务端合同；应明确标记为 `deferred`，转而继续推进 Nostr 聊天主链。
6. 当前允许继续做的重点只包括:
   - 本地 `nsec / hexKey` 登录与签名
   - `bunker / NIP-46` 远端 signer 握手与签名
   - 基于公共 `wss` relay 的 Nostr 聊天 MVP，优先支持 `damus / nos / primal / 0xchat` 这类公开 relay 入口
   - relay 连接、发布、订阅、增量同步、回执与去重
   - Nostr direct/group message 映射、持久化与会话展示
   - 不依赖后端的消息域模型收敛与错误处理
7. `Nostr 聊天主链` 的完成标准里，不接受“必须用户手动进入 diagnostics 或先点一次 `Connect` 才能工作”这种前提；登录后、选圈后、建圈后，runtime auto-start / auto-recover 必须默认成立。
8. 若“完整复刻 `tmp/xchat-app-main`”与“先出简单 P2P 聊天 MVP”发生冲突，以 `MVP 范围优先`:
   - 先实现 `MVP` 必需页面和交互
   - `subscription / checkout / paid private circle` 等非 MVP 页面不进入主流程
   - 待 MVP 聊天闭环稳定后，再决定是否补齐源项目里的非 MVP 页面
...
8. `UI 完全一样` 的要求，当前只适用于 `MVP` 保留范围内的页面与流程；不要为了追求全量页面复刻，把 `订阅 / 支付 / private cloud` 之类非 MVP 模块重新带回主流程。
9. 除非用户明确要求做 GUI 运行验收，否则不要依赖不可控的桌面自动点按、自动截图或无监督界面操作来下结论；优先做源码对照、静态实现和可复现的构建/测试验证。
...
6. 若任务选择出现冲突，优先级固定为:
   ...
   - `消息同步/回执/持久化`
   - `MVP 范围内页面的 source-like UI 对齐`
   - 其他全部延后，尤其是任何后端依赖能力或非 MVP 页面复刻
